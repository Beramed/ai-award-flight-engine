...
func _physics_process(delta: float) -> void:
	if exploding:
		...
		_hurt_in_blast()
		if boom_t >= GROUND_BOOM_FRAMES / BOOM_FPS and $Boom.monitoring:
			call_deferred("_disarm_blast")
		return
...
func _arm_blast() -> void:
	if not is_instance_valid(self):
		return
	$CollisionShape2D.disabled = true
	var shape := CircleShape2D.new()
	shape.radius = 72.0 if ground_blast else 42.0
	$Boom/CollisionShape2D.shape = shape
	$Boom/CollisionShape2D.position = Vector2(0, 6) if ground_blast else Vector2.ZERO
	$Boom.monitoring = true
	_hurt_in_blast()


func _disarm_blast() -> void:
	if not is_instance_valid(self):
		return
